            ...
            <div class="flex items-center gap-x-1">
                ...
                <x-admin::dropdown :class="$channels->count() <= 1 ? 'hidden' : ''">
                    ...
                    <x-slot:content class="!p-0">
                        @php
                            $localeCode = $currentLocale?->code ?? 'sr';
                        @endphp

                        @foreach ($channels as $channel)
                            <a
                                 href="?{{ Arr::query(['channel' => $channel->code, 'locale' => $localeCode]) }}"
                ...
                <x-admin::dropdown :class="$currentChannel->locales->count() <= 1 ? 'hidden' : ''">
                    ...
                    <x-slot:toggle>
                        ...
                        >
                            ...
                            @php
                                $localeName = $currentLocale?->name ?? 'Srpski';
                                $localeCode = $currentLocale?->code ?? 'sr';
                            @endphp
                    ...
                    <x-slot:content class="!p-0">
                       ...
                       @foreach ($currentChannel->locales->sortBy('name') as $locale)
                            <a
                                ...
                                class="flex gap-2.5 px-5 py-2 text-base cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-950 dark:text-white {{ $locale->code == $localeCode ? 'bg-gray-100 dark:bg-gray-950' : '' }}"
                            >
                                {{ $locale->name ?? 'Jezik' }}
                            </a>
                        @endforeach
