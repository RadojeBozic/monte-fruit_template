...
@pushOnce('scripts')
        ...
        <div class="flex flex-1 flex-col gap-2 max-xl:flex-auto">
            <div class="box-shadow rounded bg-white p-4 dark:bg-gray-900">
                <div class="flex items-center justify-between gap-x-2.5">
                    ...
                    <!-- Add Services Button -->
                    <div class="flex gap-2.5">
                        <div class="secondary-button" @click="add">
                            @lang('admin::app.settings.themes.edit.services-content.add-btn')
                        </div>
                    </div>
                </div>
                ...
                <div class="grid pt-4" v-if="servicesContent.services.length">
                    <div v-for="(service_details, index) in servicesContent.services" class="flex cursor-pointer justify-between gap-2.5 py-5"
                        :class="{'border-b border-slate-300 dark:border-gray-800': index < servicesContent.services.length - 1}">

                        <input type="hidden" :name="'{{ $currentLocale->code }}[options]['+ index +'][title]'" :value="service_details.title" />
                        <input type="hidden" :name="'{{ $currentLocale->code }}[options]['+ index +'][description]'" :value="service_details.description" />
                        <input type="hidden" :name="'{{ $currentLocale->code }}[options]['+ index +'][service_icon]'" :value="service_details.service_icon" />

                        <div class="flex gap-2.5">
                            <div class="grid place-content-start gap-1.5">
                                <p class="text-gray-600 dark:text-gray-300">
                                    <div>@lang('admin::app.settings.themes.edit.services-content.title'): <span>@{{ service_details.title }}</span></div>
                                </p>
                                <p class="text-gray-600 dark:text-gray-300">
                                    <div>@lang('admin::app.settings.themes.edit.services-content.description'): <span>@{{ service_details.description }}</span></div>
                                </p>
                                <p class="text-gray-600 dark:text-gray-300">
                                    @lang('admin::app.settings.themes.edit.services-content.service-icon'): <span>@{{ service_details.service_icon }}</span>
                                </p>
                            </div>
                        </div>

                        <div class="flex items-center gap-x-5">
                            <p class="cursor-pointer text-blue-600 transition-all hover:underline" @click="edit(service_details)">
                                @lang('admin::app.settings.themes.edit.edit')
                            </p>
                            <p class="cursor-pointer text-red-600 transition-all hover:underline" @click="remove(service_details)">
                                @lang('admin::app.settings.themes.edit.services-content.delete')
                            </p>
                        </div>
                    </div>
                ...
                <div v-else class="grid justify-center justify-items-center gap-3.5 px-2.5 py-10">
            ...
            <x-admin::form v-slot="{ handleSubmit }" as="div">
                <form @submit="handleSubmit($event, saveServices)" ref="createServiceForm">
    ...
    <script type="module">
        app.component('v-services-content', {
            ...
            data() {
                return {
                    servicesContent: @json($theme->translate($currentLocale->code)['options'] ?? { services: [] }),
                    deletedServices: [],
                    selectedService: {},
                    isUpdating: false
                };
            ...
            methods: {
                saveServices(params, { resetForm }) {
                    this.servicesContent.services.push({
                        title: this.selectedService.title,
                        description: this.selectedService.description,
                        service_icon: this.selectedService.service_icon,
                    });
                    resetForm();
                    this.$refs.addServiceModal.toggle();
                },
                remove(service) {
                    ...
                    this.servicesContent.services = this.servicesContent.services.filter(s => s !== service);
                },
                add() {
                    this.selectedService = {};
                    this.isUpdating = false;
